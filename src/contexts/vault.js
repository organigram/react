...
export const VaultProvider = ({ children }) => {
    ...
    const loadKey = async (_setKey = false, account = null) => {
        if (!keyserver)
            throw new Error("Key load error : Keyserver not found.")
    ...
    const uploadKey = async (_key = null) => {
        if (!keyserver || !keyserver.address || !(await Organ.isOrgan(keyserver.address).catch(() => false)))
            throw new Error("Key upload error : Keyserver not found.")
        if (!_key)
            _key = key
        if (!_key)
            throw new Error("Key upload error : Key not supplied for upload.")
    ...
    const addFile = async (file, recipientsKeys = [], signatureKey = null) => {
        ...
        if (!fileCid)
            throw new Error("Unable to add encrypted file to IPFS.")
    ...
    const getFiles = async cid => {
        ...
        for(const metadata of filesMetadata) {
            ...
            if (!metadata || !metadata.cid || !metadata.encryption || !metadata.encryption.password)
                throw new Error("Metadata file is not a valid reference to encrypted data.")
