...
export const OrganProvider = ({ organ, address, children }) => {
  ...
  const load = () => {
    const _address = address != null && address !== ''
      ? address
      : organ?.address != null && organ.address !== ''
        ? organ.address
        : null
    if (_address != null) {
      setError(null)
      setLoading(true)
      manager.getOrgan(_address, false)
        .then(data => setOrgan(data))
        .catch(error => {
          console.error('Error loading organ ', _address, error.message)
          setOrgan(null)
          setError(error)
        })
