...
export const Vault = () => {
    ...
    const togglePins = () => {
        if (showPins) { setShowPins(false) } else if (hasSignature) { setShowPins(true) } else {
 _createSignature()
        ...
        <>
            ...
            <div>
                {hasSignature ? (
                    ...
                    <button onClick={() => {
                        _createSignature()
                        .then(k => k && console.info('Signature generated, encryption key can be unlocked.'))
                        .catch(error => console.warn(error.message))
                ...
                {!keyserver && (
                    <button onClick={() => {
                        _deployKeyserver()
                        .then(k => k && console.info('Keyserver deployed', k.address))
                        .catch(error => console.warn(error.message))
                ...
                {!key && (
                    <button onClick={() => {
                        createKey(true)
                        .then(k => console.info('key saved', k))
                ...
                {!key && keyUploaded && (
                    <button onClick={() => {
                        loadKey(true)
                        .then(k => console.info('key loaded', k))
                ...
                {key && !keyUploaded && (
                    <button onClick={() => {
                        uploadKey()
                        .then(k => console.info('key uploaded', k))
...
export const WithEncryptionCheckbox = () => {
    ...
    if (typeof hasSignature === 'undefined') { return <p className="text-info">Loading signature.</p> }
    if (!hasSignature) {
 return (
            <></>
        )
