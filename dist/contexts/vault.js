...
import { openpgp, ipfsNode, web3, ecRecover, sign, generateSignature, generatePassword, _encryptMessagePGP, _decryptMessagePGP, generateKey, getAccount, Keyserver, Organ, EMPTY_CID } from '@organigram/client-js';
...
export const VaultProvider = ({
  ...
  const {
    account,
    network
  } = useWeb3(); // @todo : Use state for multiple keyservers.
  ...
  const createKey = async (_setKey = false) => {
    ...
    const signature = sessionStorage.getItem(`organigram-vault-signature-${account}`.toLowerCase()) || (await _createSignature());
  ...
  const addFile = async (file, recipientsKeys = [], signatureKey = null) => {
    ...
    const encrypted = message.packets.write(); // Upload encrypted file to IPFS.

    const ipfs = await ipfsNode;
    const fileCid = await ipfs.add(encrypted).then(result => result.cid).catch(error => {
  ...
  const getFiles = async cid => {
    ...
    const ipfs = await ipfsNode;
    const filesMetadata = await getFilesMetadata(cid).catch(_error => []);
    const files = [];
    ...
    for (const metadata of filesMetadata) {
      ...
      for await (const file of ipfs.get(metadata.cid)) {
        ...
        const cipherfile = concat(fileContent); // Decrypt file. clearbits is Uint8Array
        ...
        const clearfile = new File([clearbits], `${metadata.name}`, {
          type: metadata.type
        });
  ...
  const getFilesMetadata = async cid => {
    ...
    for await (const metadataFile of ipfs.get(cid)) {
      ...
      const ciphermetadata = concat(metadataContent); // @todo : Detect if file is an encrypted JSON.
...
export const withVaultProvider = ComposedComponent => props => /*#__PURE__*/React.createElement(VaultProvider, null, /*#__PURE__*/React.createElement(ComposedComponent, props));