...
export const ProcedureProvider = ({
  ...
  const reloadMetadata = React.useCallback(async () => {
    if (_procedure && _procedure.reloadMoves) _procedure.reloadMetadata().then(p => setProcedure(_prev => p));
  }, [_procedure]);
  const reloadMove = React.useCallback(async moveKey => {
    if (_procedure && _procedure.reloadMoves) _procedure.reloadMove(moveKey).then(p => setProcedure(_prev => p));
  }, [_procedure]);
