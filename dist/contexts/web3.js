...
export const Web3Provider = ({
    ...
    if (timer.current === null) {
      timer.current = setInterval(() => {
        ...
        (async () => {
          ...
          if (typeof window.web3 === 'undefined' || typeof window.web3.currentProvider === 'undefined') setStatus(ETHEREUM_UNAVAILABLE);else if (!_web3) setWeb3(new Web3(window.web3.currentProvider));else {
            if (window.ethereum && window.ethereum.autoRefreshOnNetworkChange) window.ethereum.autoRefreshOnNetworkChange = false;
            const accounts = await _web3.eth.getAccounts();
            if (accounts.length === 0 && status !== ETHEREUM_LOCKED) setStatus(ETHEREUM_LOCKED);

            if (accounts.length > 0) {
              if (status !== ETHEREUM_UNLOCKED) setStatus(ETHEREUM_UNLOCKED);
              const newAccount = accounts[0].toLowerCase(); // Update account.
              ...
              if (newAccount) {
                _web3.eth.getBalance(newAccount, (error, data) => {
                  if (!error) {
                    ...
                    if (newBalance && (!balance || balance && !newBalance.eq(balance))) {
                      setBalance(_web3.utils.fromWei(data, 'ether').toString().substring(0, 6));
                    }
                  }
  ...
  const ecRecover = _web3 && _web3.eth && _web3.eth.personal && _web3.eth.personal.ecRecover ? _web3.eth.personal.ecRecover : null;
  const sign = _web3 && _web3.eth && _web3.eth.personal && _web3.eth.personal.sign ? _web3.eth.personal.sign : null;
